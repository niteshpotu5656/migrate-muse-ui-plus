...
export const TargetDBForm: React.FC = () => {
  ...
  const handleSelectDB = (dbType: string) => {
    updateState({
      targetConfig: { ...state.targetConfig, type: dbType }
    });
  };
    ...
    <div className="space-y-6">
      <CardHeader>
        ...
        <CardDescription className="text-gray-400">
          Choose the type of database you want to migrate to
        </CardDescription>
      </CardHeader>
      <CardContent>
          ...
          {databaseOptions.map((db) => (
            ...
            >
              ...
              <div className="flex flex-col items-center justify-center py-4">
                <div className={`text-${db.id === state.targetConfig.type ? 'blue' : 'gray'}-400`}>
                  {db.icon}
                </div>
